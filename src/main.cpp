...
template <typename T>
static void fisher_yates_shuffle(std::vector<T>& t) {
...
static uint16_t pop_tree_idx(const std::vector<uint16_t>& list,
                             size_t& pos,
...
static void run_generate(int argc, char* argv[]) {
    ...
    for (int level = 2; level <= 50; level++) {
        if (level % 5 == 0) learn(level, "class");
        if (level % 10 == 2) learn(level, "generic");
    }

    // Compute addons fingerprint from auto-detected sources
    std::string fingerprint =
        compute_fingerprint(config.version, lists.addon_sources,
                            config.addons);
...
static void run_search(int argc, char* argv[]) {
    if (argc < 4) {
        std::fprintf(stderr,
                     "Usage: %s search <unlocks.json> <rules.json>\n",
                     argv[0]);
        std::exit(1);
    }
    ...
    auto intern = [&](const std::string& s) -> uint16_t {
        ...
        if (it != tree_to_idx.end()) return it->second;
    ...
    // Levels 2-50
    for (int level = 2; level <= 50; level++) {
        if (level % 5 == 0) schedule_template.push_back({level, true});
        if (level % 10 == 2) schedule_template.push_back({level, false});
    }

    // Find the highest max_level across all rules so we can stop early.
    int max_needed_level = 0;
    for (const auto& r : indexed_rules)
        if (r.max_level > max_needed_level) max_needed_level = r.max_level;
    ...
    std::string fingerprint =
        compute_fingerprint(config.version, base.addon_sources,
                            config.addons);
    ...
    for (unsigned tid = 0; tid < nthreads; tid++) {
        ...
        uint32_t end = (tid == nthreads - 1) ? MAX_SEED
                                              : (tid + 1) * chunk;
        ...
        threads.emplace_back([&, tid, start, end]() {
            ...
            for (uint32_t seed = start; seed <= end; seed++) {
                ...
                for (int si = 0; si < schedule_len; si++) {
                    ...
                    auto& list = schedule_template[si].is_class
                                     ? class_idx : generic_idx;
                    size_t& pos = schedule_template[si].is_class
                                      ? cpos : gpos;
                    uint16_t tree = pop_tree_idx(list, pos, seen);
                    if (tree == UINT16_MAX) continue;
                ...
                uint64_t count = found.fetch_add(
